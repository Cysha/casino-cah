<?php
namespace NoelDavies\CardsAgainstHumanity\Providers;

use Cysha\Casino\Cards\Contracts\CardProvider;

class CAHwwwCardProvider implements CardProvider
{
    const WHITE_CARDS = [
        ["text" => "Don\'t worry, Penny! Go Go Gadget _!", "pick" => 1],
["text" => "I need you like _ needs _.", "pick" => 2],
["text" => "I\'m just gonna stay in tonight. You know, Netflix and _.", "pick" => 1],
["text" => "Nothing says \"I love you\" like _.", "pick" => 1],
["text" => "Such _. Very _. Wow.", "pick" => 2],
["text" => "This app is basically Tinder, but for _.", "pick" => 1],
["text" => "TRIGGER WARNING: _.", "pick" => 1],
["text" => "What did I nickname my genitals?", "pick" => 1],
["text" => "You guys, you can buy _ on the dark web.", "pick" => 1]
    ];

    const BLACK_CARDS = [
        ["text" => "A complete inability to understand anyone else\'s perspective."],
["text" => "A fun, sexy time at the nude beach."],
["text" => "A man from Craigslist."],
["text" => "A night of Taco Bell and anal sex."],
["text" => "A respectful discussion of race and gender on the Internet."],
["text" => "Cat massage."],
["text" => "Destroying Dick Cheney\'s last horcrux."],
["text" => "<i>Game of Thrones</i> spoilers."],
["text" => "Getting teabagged by a fifth grader in Call of Duty."],
["text" => "Goats screaming like people."],
["text" => "Googling."],
["text" => "Internet porn analysis paralysis."],
["text" => "Matching with Mom on Tinder."],
["text" => "My browser history."],
["text" => "My privileged white penis."],
["text" => "Pretending to be black."],
["text" => "Smash Mouth."],
["text" => "Taking a shit while running at full speed."],
["text" => "That thing on the Internet everyone\'s talking about."],
["text" => "Three years of semen in a shoebox."],
["text" => "YouTube comments."]
    ];

    /**
     * Provides the cards for a deck.
     *
     * @return array an array of cards
     */
    public function getCards()
    {
        return [
            'WHITE' => self::WHITE_CARDS,
            'BLACK' => self::BLACK_CARDS,
        ];
    }
}