<?php
namespace NoelDavies\CardsAgainstHumanity\Providers;

use Cysha\Casino\Cards\Contracts\CardProvider;

class CAHscienceCardProvider implements CardProvider
{
    const WHITE_CARDS = [
        ["text" => "In line with our predictions, we find a robust correlation between _ and _ (p&gt;.05).", "pick" => 2],
["text" => "In what\'s being hailed as a major breakthrough, scientists have synthesized _ in the lab.", "pick" => 1],
["text" => "A study published in Nature this week found that _ is good for you in small doses.", "pick" => 1],
["text" => "In an attempt to recreate conditions just after the Big Bang, physicists at the LHC are observing collisions between _ and _.", "pick" => 2],
["text" => "What really killed the dinosaurs?", "pick" => 1],
["text" => "Hey there, Young Scientists! Put on your labcoats and strap on your safety goggles, because today we\'re learning about _!", "pick" => 1],
["text" => "Today on MythBusters, we found out how long _ can withstand _.", "pick" => 2]
    ];

    const BLACK_CARDS = [
        ["text" => "A supermassive black hole."],
["text" => "Reconciling quantum theory with general relativity."],
["text" => "Electroejaculating a capuchin monkey."],
["text" => "Insufficient serotonin."],
["text" => "Evolving a labyrinthine vagina."],
["text" => "Getting really worried about global warming for a few seconds."],
["text" => "Infinity."],
["text" => "Oxytocin release via manual stimulation of the nipples."],
["text" => "Uranus."],
["text" => "Being knowledgeable in a narrow domain that nobody understands or cares about."],
["text" => "Achieving reproductive success."],
["text" => "Slowly evaporating."],
["text" => "The quiet majesty of the sea turtle."],
["text" => "A 0.7 waist-to-hip ratio."],
["text" => "Fun and interesting facts about rocks."],
["text" => "Photosynthesis."],
["text" => "Developing secondary sex characteristics."],
["text" => "Failing the Turing test."],
["text" => "Explosive decompression."],
["text" => "Driving into a tornado to learn about tornadoes."],
["text" => "David Attenborough watching us mate."],
["text" => "3.7 billion years of evolution."],
["text" => "The Sun engulfing the Earth."]
    ];

    /**
     * Provides the cards for a deck.
     *
     * @return array an array of cards
     */
    public function getCards()
    {
        return [
            'WHITE' => self::WHITE_CARDS,
            'BLACK' => self::BLACK_CARDS,
        ];
    }
}